...
class Controller {
 public:
  ...
  const std::vector<std::string> &mimic_names() const {
    return mimic_dof_names_;
  }
  ...
  std::unordered_map<std::string, tsid::math::Vector>
  activated_contacts_forces() {
    return activated_contacts_forces_;
  };
  ...
  virtual const tsid::math::Vector3 &com() const {
    return robot_->com(tsid_->data());
  }

  virtual const tsid::math::Vector3 momentum() const { return momentum_; }

  const std::vector<int> &non_mimic_indexes() const {
    return non_mimic_indexes_;
  }
  Eigen::VectorXd filter_cmd(const Eigen::VectorXd &cmd) const {
    return utils::slice_vec(cmd, non_mimic_indexes_);
  }
  ...
  const std::vector<double> &pinocchio_model_cumulated_masses() {
    return tsid_->data().mass;
  };
  const std::vector<std::string> &pinocchio_joint_names() const {
    return robot_->model().names;
  }
  const pinocchio::SE3 &model_joint_pos(const std::string &joint_name) const {
    assert(tsid_);
    assert(robot_);
    IWBC_ASSERT(robot_->model().existJointName(joint_name), "[", joint_name,
                "] (joint) does not exist!");
    return robot_->position(tsid_->data(),
                            robot_->model().getJointId(joint_name));
  }
  pinocchio::SE3 model_frame_pos(const std::string &frame_name) const {
    assert(tsid_);
    assert(robot_);
    IWBC_ASSERT(robot_->model().existFrame(frame_name), "[", frame_name,
                "] (frame) does not exist!");
    return robot_->framePosition(tsid_->data(),
                                 robot_->model().getFrameId(frame_name));
  }
  double cost(const std::shared_ptr<tsid::tasks::TaskBase> &task) const {
    assert(task);
    return (task->getConstraint().matrix() * ddq_ -
            task->getConstraint().vector())
        .norm();
  }
  ...
  void save_configuration(const std::string &config_name,
                          const std::string &robot_name = "robot") const;
  ...
  const std::string &floating_base_joint_name() const {
    return floating_base_joint_name_;
  }

  // check if pinocchio model is colliding
  inria_wbc::utils::CollisionCheck collision_check() {
    return collision_check_;
  }
  ...
  const void qp_step_back(const Eigen::VectorXd &q, const Eigen::VectorXd &dq,
                          const pinocchio::Data &data);
  const void qp_step_back() {
    qp_step_back(q_tsid_prev_, v_tsid_prev_, *data_prev_);
  };
 ...
 protected:
  ...
  std::vector<std::string>
      tsid_joint_names_;  // contain floating base and mimics
  ...
  std::unordered_map<std::string, tsid::math::Vector>
      activated_contacts_forces_;  // tsid contact forces of the activated
                                   // contacts
  ...
  bool send_cmd_ = true;
  Eigen::VectorXd q_solver_;  // q computed by the qp solver even when not sent
                              // (with send_cmd_ == false)

        ...
        bool WalkOnSpot::update()
        {
            ...
            case States::LIFT_UP_LF:
                if (time_ == 0) {
                    ...
                    current_foot_trajectory_ = trajectory_handler::compute_traj(lf_init, lf_final, dt_, traj_foot_duration_);
                }
                // add left foot task
                controller->set_se3_ref(current_foot_trajectory_[time_], "lf");
                _last_lf = current_foot_trajectory_[time_];
                break;
            ...
            case States::LIFT_UP_RF:
                if (time_ == 0) {
                    ...
                    current_foot_trajectory_ = trajectory_handler::compute_traj(rf_init, rf_up, dt_, traj_foot_duration_);
                }
                // add right foot task
                controller->set_se3_ref(current_foot_trajectory_[time_], "rf");
                _last_rf = current_foot_trajectory_[time_];
                break;
            case States::LIFT_DOWN_RF:
                if (time_ == 0) {
                    ...
                    current_foot_trajectory_ = trajectory_handler::compute_traj(rf_init, rf_down, dt_, traj_foot_duration_);
                }
                // add right foot task
                controller->set_se3_ref(current_foot_trajectory_[time_], "rf");
                _last_rf = current_foot_trajectory_[time_];
            ...
            static std::ofstream ofs("com.dat");
            ofs << controller->get_pinocchio_com().transpose() << std::endl;
            static std::ofstream ofs_ref("com_ref.dat");
            ofs_ref << current_com_trajectory_[time_].transpose() << std::endl;

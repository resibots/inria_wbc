...
std::shared_ptr<tsid::tasks::TaskBase> make_se3(
    ...
    const YAML::Node &controller_node,
    const std::unordered_map<std::string,
                             std::shared_ptr<tsid::contacts::ContactBase>>
        &contact_map) {
  ...
  // convert the mask
  IWBC_ASSERT(mask_str.size() == 6,
              "SE3 masks needs to be 6D (x y z r p y), here:", mask_str);
  auto mask = convert_mask<6>(mask_str);

  // create the task
  assert(tsid);
  assert(robot);
  auto task = std::make_shared<tsid::tasks::TaskSE3Equality>(task_name, *robot,
                                                             tracked);
  ...
  if (joint && body)
    throw IWBC_EXCEPTION("Ambiguous name to track for task ", task_name,
                         ": this is both a joint and a frame [", tracked, "]");
  if (!joint && !body && !frame)
  ...
  if (joint)
    ...
    ref =
        robot->framePosition(tsid->data(), robot->model().getFrameId(tracked));
...
RegisterYAML<tsid::tasks::TaskSE3Equality> __register_se3_equality("se3",
                                                                   make_se3);
...
std::shared_ptr<tsid::tasks::TaskBase> make_com(
  ...
  IWBC_ASSERT(mask_str.size() == 3,
              "CoM masks needs to be 3D (x y z), here:", mask_str);
...
RegisterYAML<tsid::tasks::TaskComEquality> __register_com_equality("com",
                                                                   make_com);
...
std::shared_ptr<tsid::tasks::TaskBase> make_momentum(
  ...
  auto mask_str = IWBC_CHECK(node["mask"].as<std::string>());
  IWBC_ASSERT(mask_str.size() == 6,
              "Momentum mask needs to be 6D (angular_x, angular_y, angular_z, "
              "linear_x, linear_y, linear_z), here:",
              mask_str);
  auto mask = convert_mask<6>(mask_str);
...
RegisterYAML<tsid::tasks::TaskMEquality> __register_linear_momentum_equality(
...
std::shared_ptr<tsid::tasks::TaskBase> make_posture(
  ...
  auto weight = IWBC_CHECK(node["weight"].as<double>());
  auto ref_name =
      IWBC_CHECK(controller_node["CONTROLLER"]["ref_config"].as<std::string>());

  IWBC_ASSERT(robot->model().referenceConfigurations.count(ref_name) == 1,
              "Reference name ", ref_name, " not found");
  ...
  // create the task
  auto task =
      std::make_shared<tsid::tasks::TaskJointPosture>(task_name, *robot);
  ...
  if (node["mask"] && node["joint_names"])
    IWBC_ERROR(
        "You need to specify mask or joint_names to create a mask. Not both.");
  ...
  if (!node["mask"]) {
    ...
    IWBC_ASSERT(mask.size() == mask_post.size(),
                "wrong size in posture mask, expected:", mask_post.size(),
                " got:", mask.size());
...
RegisterYAML<tsid::tasks::TaskJointPosture> __register_posture("posture",
                                                               make_posture);
...
std::shared_ptr<tsid::tasks::TaskBase> make_torque(
  ...
  // create the task
  auto task =
      std::make_shared<tsid::tasks::TaskActuationEquality>(task_name, *robot);
  ...
  if (!node["mask"]) {
    ...
    auto mask = IWBC_CHECK(node["mask"].as<std::string>());
    IWBC_ASSERT(mask.size() == mask_post.size(),
                "wrong size in torque mask, expected:", mask_post.size(),
                " got:", mask.size());
    mask_post = convert_mask<Eigen::Dynamic>(mask);
  }
  task->mask(mask_post);
  ...
  if (node["scaling"]) {
    ...
    IWBC_ASSERT(scaling.size() == n_actuated,
                "wrong size in torque scaling, expected:", n_actuated,
                " got:", scaling.size());
    Eigen::VectorXd scaling_post =
        Eigen::VectorXd::Map(scaling.data(), scaling.size());
...
RegisterYAML<tsid::tasks::TaskActuationEquality> __register_torque("torque",
                                                                   make_torque);
...
std::shared_ptr<tsid::tasks::TaskBase> make_cop_equality(
  ...
  auto weight = IWBC_CHECK(node["weight"].as<double>());
  auto contact_names =
      IWBC_CHECK(node["contact_name"].as<std::vector<std::string>>());

  std::vector<Eigen::Vector3d> refs;
  for (auto &contact_name : contact_names) {
    IWBC_ASSERT(contact_map.find(contact_name) != contact_map.end(),
                "cop task : contact : ", contact_name,
                " is not inside the contact map");
    auto contact_ref =
        robot
            ->framePosition(
                tsid->data(),
                contact_map.at(contact_name)->getMotionTask().frame_id())
            .translation();
    contact_ref[2] = 0.0;
  ...
  // set the reference in our case the initial position has the cop at the
  // middle of the convex hull
  task->setReference(ref);
...
RegisterYAML<tsid::tasks::TaskCopEquality> __register_cop_equality(
    "contact-cop-equality", make_cop_equality);

////// Task contact force equality not added in the task factory because it
/// needs already created contacts //////
std::shared_ptr<tsid::tasks::TaskBase> make_contact_force_equality(
  ...
  bool floating_base_flag = (robot->na() == robot->nv()) ? false : true;
  IWBC_ASSERT(contact_map.find(contact_name) != contact_map.end(),
              "contact force equality : contact : ", contact_name,
              " is not inside the contact map");
  // create the task
  auto task = std::make_shared<tsid::tasks::TaskContactForceEquality>(
      task_name, *robot, dt, *contact_map.at(contact_name));
  ...
  if (!node["mask"]) {
    ...
    auto mask = IWBC_CHECK(node["mask"].as<std::string>());
    IWBC_ASSERT(mask.size() == 6, "wrong size in torque mask, expected:", 6,
                " got:", mask.size());
  ...
  // set the reference to nothing, it will not work until we give it the correct
  // ref + measurment
...
RegisterYAML<tsid::tasks::TaskContactForceEquality>
    __register_contact_force_equality("contact-force-equality",
                                      make_contact_force_equality);
...
std::shared_ptr<tsid::tasks::TaskBase> make_cp_inequality(
  ...
  // create the task
  auto task = std::make_shared<tsid::tasks::TaskCapturePointInequality>(
      task_name, *robot, factor * dt);
...
RegisterYAML<tsid::tasks::TaskCapturePointInequality> __register_cp_inequality(
    "contact-cp-inequality", make_cp_inequality);
...
std::shared_ptr<tsid::tasks::TaskBase> make_bounds(
  ...
  // create the task
  auto task = std::make_shared<tsid::tasks::TaskJointPosVelAccBounds>(
      task_name, *robot, dt, false);
...
RegisterYAML<tsid::tasks::TaskJointPosVelAccBounds> __register_bounds(
    "bounds", make_bounds);
...
std::shared_ptr<tsid::tasks::TaskBase> make_actuation_bounds(
    ...
    const std::unordered_map<
        std::string, std::shared_ptr<tsid::contacts::ContactBase>> &contacts) {
  assert(tsid);
  assert(robot);
  ...
  // create the task
  auto task =
      std::make_shared<tsid::tasks::TaskActuationBounds>(task_name, *robot);
...
RegisterYAML<tsid::tasks::TaskActuationBounds> __register_actuation_bounds(
    "actuation-bounds", make_actuation_bounds);
...
std::shared_ptr<tsid::contacts::Contact6dExt> make_contact_task(
  ...
  contact_points << -lxn, -lxn, lxp, lxp, -lyn, lyp, -lyn, lyp, lz, lz, lz, lz;
  Eigen::Vector3d contact_normal(normal.data());
  auto contact_task = std::make_shared<tsid::contacts::Contact6dExt>(
      task_name, *robot, tracked, contact_points, contact_normal, mu, fmin,
      fmax);
  ...
  if (joint)
    contact_ref =
        robot->position(tsid->data(), robot->model().getJointId(tracked));
  else
    contact_ref =
        robot->framePosition(tsid->data(), robot->model().getFrameId(tracked));
...
std::shared_ptr<tsid::tasks::TaskBase> make_self_collision(
  ...
  for (const auto &a : IWBC_CHECK(node["avoided"])) {
    IWBC_ASSERT(robot->model().existFrame(a.first.as<std::string>()), "Frame ",
                a.first.as<std::string>(), " in ", task_name,
                " does not exists.");
  ...
  assert(tsid);
  assert(robot);
  auto task = std::make_shared<tsid::tasks::TaskSelfCollision>(
      task_name, *robot, tracked, avoided, radius, margin, m);
  task->Kp(kp);
  task->Kd(kd);
...
RegisterYAML<tsid::tasks::TaskSelfCollision> __register_self_collision(
    "self-collision", make_self_collision);

///// measured_force is not a task
std::shared_ptr<tsid::contacts::MeasuredForceBase> make_measured_force(
    const std::shared_ptr<robots::RobotWrapper> &robot,
    const std::shared_ptr<InverseDynamicsFormulationAccForce> &tsid,
    const std::string &force_name, const YAML::Node &node,
    const YAML::Node &controller_node) {
  assert(tsid);
  assert(robot);
  ...
  auto tracked = IWBC_CHECK(node["tracked"].as<std::string>());
  bool frame = robot->model().existFrame(tracked);
  if (!frame) throw IWBC_EXCEPTION("Unknown frame [", tracked, "]");

  auto dimension = IWBC_CHECK(node["dimension"].as<std::string>());
  if (dimension == "force") {
    auto measured_force = std::make_shared<tsid::contacts::Measured3Dforce>(
        force_name, *robot, tracked);
    // add the measured force
    tsid->addMeasuredForce(*measured_force);
    return measured_force;
  } else if (dimension == "wrench") {
    auto measured_force = std::make_shared<tsid::contacts::Measured6Dwrench>(
        force_name, *robot, tracked);
    // add the measured force
    tsid->addMeasuredForce(*measured_force);
    return measured_force;
  } else
    throw IWBC_EXCEPTION("Unknown dimension [", dimension,
                         "]. Must be 'force' or 'wrench'");
}

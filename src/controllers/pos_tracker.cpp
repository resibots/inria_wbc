...
PosTracker::PosTracker(const YAML::Node &config) : Controller(config) {
  ...
  pinocchio::srdf::loadReferenceConfigurations(robot_->model(), p_srdf,
                                               verbose_);

  // q_tsid_ for talos is of size 37 (pos+quat+nactuated)
  auto ref_map = robot_->model().referenceConfigurations;
  IWBC_ASSERT(
      ref_map.find(ref_config) != ref_map.end(),
      "The following reference config is not in ref_map : ", ref_config);
  q_tsid_ = ref_map[ref_config];
  if (verbose_)
    std::cout << "q_tsid ref:" << q_tsid_.transpose() << "[" << q_tsid_.size()
              << "]" << std::endl;
  ...
  if (floating_base_) {
    ...
    q0_ << q_tsid_.head(3), aaxis.angle() * aaxis.axis(),
        q_tsid_.tail(robot_->na());
...
#ifdef TSID_QPMAD_FOUND
    solver_ = solver_t(solvers::SolverHQPFactory::createNewSolver(
        solvers::SOLVER_HQP_QPMAD, "solver-qpmad"));
#else
    IWBC_ERROR("'qpmad' solver is not available in tsid or in the system.");
#endif
  } else if (solver_to_use_ == "eiquadprog") {
    solver_ = solver_t(solvers::SolverHQPFactory::createNewSolver(
        solvers::SOLVER_HQP_EIQUADPROG_FAST, "solver-eiquadprog"));
  } else {
    IWBC_ERROR(
        "solver in configuration file must be either 'eiquadprog' or 'qpmad'.");
  }
  ...
  if (c["joint_range_reduction"]) {
    double reduction_rads =
        IWBC_CHECK(c["joint_range_reduction"].as<double>()) / 180 * M_PI;
    if (reduction_rads < 0)
    ...
    if (verbose_)
      std::cout << "Joints' range of motion reduced by: " << reduction_rads
                << " rads." << std::endl;
    ...
    for (size_t i = 0; i < robot_->na(); ++i) {
      if (q_ub[i] - q_lb[i] < 2 * reduction_rads)
        IWBC_ERROR(
            "Joint range reduction: reduction cannot be greater than actual "
            "range of motion");
      ...
      if (verbose_)
        std::cout << "to (" << q_lb[i] << "," << q_ub[i] << "). q=" << q[i]
                  << std::endl;

      if (q[i] < q_lb[i] || q[i] > q_ub[i])
        IWBC_ERROR("Joint range reduction(", pinocchio_joint_names()[i + 2],
                   "): q0 falls outside reduced joint space.");
    }
  ...
  if (verbose_) {
    ...
    std::cout << "total number of variable (acceleration + contact-force) : "
              << tsid_->nVar() << std::endl;
    std::cout << "number of equality constraints : " << tsid_->nEq()
              << std::endl;
    std::cout << "number of inequality constraints : " << tsid_->nIn()
              << std::endl;
    std::cout << "--------- ------------- ---------" << std::endl;
...
void PosTracker::parse_tasks(const std::string &path,
  ...
  int task_count = 0;
  if (verbose_) std::cout << "parsing task file:" << path << std::endl;
  ...
  for (auto it = task_list.begin(); it != task_list.end(); ++it) {
    ...
    if (type == "contact") {
      ...
      auto task =
          tasks::make_contact_task(robot_, tsid_, name, it->second, config);
      contacts_[name] = task;
      activated_contacts_.push_back(name);
      all_contacts_.push_back(name);
    } else if (type == "measured-force") {
      auto force =
          tasks::make_measured_force(robot_, tsid_, name, it->second, config);
      ...
      auto task = tasks::FactoryYAML::instance().create(
          type, robot_, tsid_, name, it->second, config, {});
      tasks_[name] = task;
      activated_tasks_.push_back(name);
    }
    if (verbose_)
      std::cout << "added task/contact:" << name << " type:" << type
                << std::endl;
    task_count++;
  }
  for (auto it = task_list.begin(); it != task_list.end(); ++it) {
    ...
    if (type.find("contact-") != std::string::npos) {
      auto task = tasks::FactoryYAML::instance().create(
          type, robot_, tsid_, name, it->second, config, contacts_);
      tasks_[name] = task;
      activated_tasks_.push_back(name);
    }
    if (verbose_)
...
void PosTracker::parse_frames(const std::string &path) {
  if (verbose_) std::cout << "Parsing virtual frame file:" << path << std::endl;
  ...
  for (auto it = node.begin(); it != node.end(); ++it) {
    ...
    robot_->model().addFrame(
        pinocchio::Frame(name, frame.parent, parent_frame_id,
...
void PosTracker::set_se3_ref(const pinocchio::SE3 &ref,
...
void PosTracker::set_contact_se3_ref(const pinocchio::SE3 &ref,
                                     const std::string &contact_name) {
  auto c = std::dynamic_pointer_cast<tsid::contacts::Contact6dExt>(
      contact(contact_name));
  auto sample = trajs::to_sample(ref);
  c->setReference(sample);
}

void PosTracker::set_se3_ref(tsid::trajectories::TrajectorySample &sample,
                             const std::string &task_name) {
  auto task = se3_task(task_name);
  task->setReference(sample);
}

void PosTracker::set_contact_se3_ref(
...
void PosTracker::set_measured_6Dwrench(const tsid::math::Vector6 &f_ext,
                                       const std::string &measured_force_name) {
  auto force = std::dynamic_pointer_cast<tsid::contacts::Measured6Dwrench>(
      measured_force(measured_force_name));
  force->setMeasuredContactForce(f_ext);
}

void PosTracker::set_measured_3Dforce(const tsid::math::Vector3 &f_ext,
                                      const std::string &measured_force_name) {
  auto force = std::dynamic_pointer_cast<tsid::contacts::Measured3Dforce>(
      measured_force(measured_force_name));
  force->setMeasuredContactForce(f_ext);
}

void PosTracker::remove_contact(const std::string &contact_name) {
  if (verbose_) std::cout << "removing contact:" << contact_name << std::endl;
  IWBC_ASSERT(contacts_.find(contact_name) != contacts_.end(),
              "Trying to remove an contact:", contact_name);
  bool res = tsid_->removeRigidContact(contact_name);
  IWBC_ASSERT(res, " contact ", contact_name, " not found");
  activated_contacts_.erase(
      std::remove(activated_contacts_.begin(), activated_contacts_.end(),
                  contact_name),
      activated_contacts_.end());
}

void PosTracker::add_contact(const std::string &contact_name) {
  if (verbose_) std::cout << "adding contact:" << contact_name << std::endl;
...
Eigen::VectorXd PosTracker::force_torque_from_solution(
    const std::string &contact_name, float foot_mass,
    const std::string &sole_frame) {
  Eigen::Matrix<double, 6, 1> force_tsid;
  force_tsid.setZero();

  IWBC_ASSERT(activated_contacts_forces_.find(contact_name) !=
                  activated_contacts_forces_.end(),
              contact_name, " not in activated_contacts_forces_");
  auto contact_force = activated_contacts_forces_[contact_name];
  auto generatorMatrix =
      std::dynamic_pointer_cast<tsid::contacts::Contact6dExt>(
          contact(contact_name))
          ->Contact6d::getForceGeneratorMatrix();
  ...
  if (foot_mass > 1e-5) {
    ...
    auto contact_frame =
        robot_->model()
            .frames[contact(contact_name)->getMotionTask().frame_id()]
            .name;
    IWBC_ASSERT(robot_->model().existFrame(contact_frame),
                "Frame " + contact_frame + " does not exist in model.");
    IWBC_ASSERT(robot_->model().existFrame(sole_frame),
                "Frame " + sole_frame + " does not exist in model.");

    auto contact_world =
        tsid_->data().oMi[robot_->model().getJointId(contact_frame)];
    ...
    force_tsid.tail(3) -=
        (contact_world.translation() - sole_world.translation()).cross(tmp);
  }
  return force_tsid;
}
...
void PosTracker::remove_measured_force(const std::string &measured_force_name) {
  if (verbose_)
    std::cout << "removing measured_force: " << measured_force_name
              << std::endl;
  IWBC_ASSERT(
      measured_forces_.find(measured_force_name) != measured_forces_.end(),
      "Trying to remove a measured_force: ", measured_force_name);
  bool res = tsid_->removeMeasuredForce(measured_force_name);
  IWBC_ASSERT(res, " measured_force ", measured_force_name, " not found");
  activated_measured_forces_.erase(
      std::remove(activated_measured_forces_.begin(),
                  activated_measured_forces_.end(), measured_force_name),
      activated_measured_forces_.end());
}
...
void PosTracker::remove_task(const std::string &task_name,
                             double transition_duration) {
  if (verbose_) std::cout << "removing task:" << task_name << std::endl;
  IWBC_ASSERT(tasks_.find(task_name) != tasks_.end(),
              "Trying to remove an unknown task:", task_name);
  bool res = tsid_->removeTask(task_name, transition_duration);
  IWBC_ASSERT(res, "Cannot remove an unknown task: ", task_name);
  activated_tasks_.erase(
      std::remove(activated_tasks_.begin(), activated_tasks_.end(), task_name),
      activated_tasks_.end());
}

size_t PosTracker::num_task_weights() const {
  // we count all the tasks with a priority >= 1 (since it does not matter for
  // 0)
  size_t k = 0;
